...
def calcl_R1_R2_landa(fm=0.5, dk=5):
...
def calc_A(fm=0.5, dk=5):

    f = fm
    d = dk

    x = calc_X(f, d)
    r1, r2, landa = calcl_R1_R2_landa(f, d)
    R = d*1000
    b = math.atan((Er+1)/x)
    p = ((pi*R)/(landa*x))*math.cos(b)

    A1 = (2 + 0.3*p)/(2 + p + 0.6*p)

    A = A1 - (math.sin(b)*math.sqrt(p/2)*math.exp(-5*p/8))
...
def calcular_Espv(fm=0.5, dk=5):
    ...
    complejo = complex((Rvpol[0]*math.cos(new_ang)),
                       Rvpol[0]*math.sin(new_ang))

    # -------Espacial Vertical-----------

    Espv = (60/(d*1000)) * \
        math.sqrt(((1 + complejo.real)**2) + ((complejo.imag)**2))
...
def calcular_Esph(fm=0.5, dk=5):
    ...
    Esph = (60/(d*1000)) * \
        math.sqrt((((1 + complejoh.real)**2) + ((complejoh.imag)**2)))
...
def mostrarDatosGenerales(Directividad, Cte, Potencia, Frec, Distancia, h1, h2, Epsilon, Sigma, Ang_aproxRad, Ang_aproxgrd, coef_ReflexRv, angRv, coef_ReflexRh, angRh):
    ...
    tabEjemplo1 = {'Datos Generales': [
        ...
        str('%.3E' % Decimal(Ang_aproxRad)) + "  " +
            str('%.3E' % Decimal(Ang_aproxgrd)),
        str('%.3E' % Decimal(coef_ReflexRv)) +
            " " + str('%.3E' % Decimal(angRv)),
                     str('%.3E' % Decimal(coef_ReflexRh)) + "  " + str('%.3E' % Decimal(angRh))],
    }

    tab = tabulate(tabEjemplo1, headers='keys',
                   tablefmt='simple', stralign='left', floatfmt='.4f')
    print(tab)
...
def CalculosIntermedios(jfa=1, jda=5, landaa=300, da=5, R1a=1.000001, R2a=1.000002, h1a=30.4878, h2a=9.146341, Xia=0.001891795, f=0.5, era=15, sigmaa=5*10**-3, xa=90, Aa=0.9231):
    ...
    saveTabCalculosIntermedios = tabulate(
        tabCalculosIntermedios,  tablefmt='simple', stralign='left', floatfmt='.4f')
...
os.system("cls")
mostrarDatosGenerales(Directividad, Cte, Potencia, Frec, Distancia, h1, h2, Epsilon,
                        Sigma, Ang_aproxRad, Ang_aproxgrd, coef_ReflexRv, angRv, coef_ReflexRh, angRh)
...
for i in range(0, Var_Fre):
    ...
    for j in range(0, var_dis):
        ...
        Resultados_Espv.append(Espv)
        Resultados_Esuv.append(Esuv)
        Resultados_Esph.append(Esph)
        Resultados_Esuh.append(Esuh)
        Resultado_dist.append(d)

        d += 5

    Resultado_resum.append({'distancia':Resultado_dist,
                            'Espv': Resultados_Espv,
                            'Esuv': Resultados_Esuv,
                            'Esph':Resultados_Esph,
                            'Esuh': Resultados_Esuh,
                            'Frecuencia':[f]})
    ...
    CalculosIntermedios(da=d-5, R1a=R1, R2a=R2, landaa=landa, h1a=h1, h2a=h2, xa=x, f=f, sigmaa= sig, era=Er, Aa = A)
...
for x in Resultado_resum:
    
    print(tabulate(x, headers= 'keys', tablefmt='fancy_grid'))
    print('-'*70)


intruccion3 = input('preciona c para salir')
